...
const Pitch::Type Pitch::GetType() const{
    if((_Letter == Letter::InvalidID)||(_Accidental == Accidental::InvalidID))
    {
        return Pitch::InvaildPitch;
    }
    return (_Letter.ID() * 9) + (4 + _Accidental.ID());
}
...
bool MusicCompositionCore::MusicalNote::operator < (const Pitch &lhs, const Pitch &rhs)
{
...
bool MusicCompositionCore::MusicalNote::operator == (const Pitch::Type& lhs, const Pitch& rhs)
{
    if(Pitch(lhs).NotePitch() == rhs.NotePitch()){return 1;}
    return 0;
}

bool MusicCompositionCore::MusicalNote::operator == (const Pitch &lhs, const Pitch::Type &rhs)
{
